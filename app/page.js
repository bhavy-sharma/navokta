...
export default function CreateInvoicePage() {
  ...
  const [terms, setTerms] = useState(
    "Payment due within 15 days. Late payments may incur additional charges."
  );
  ...
  const displayInvoiceNumber =
    ...
    `INV-${new Date().getFullYear()}-${String(items.length).padStart(3, "0")}`;
  ...
  const upiLink = `upi://pay?pa=${encodeURIComponent(
    ...
    billFrom.name
  )}&am=${total}&cu=INR&tn=Invoice%20${displayInvoiceNumber}`;
  ...
  const addItem = () =>
    setItems([
      ...items,
      { id: Date.now().toString(), name: "", qty: 1, rate: 0, description: "" },
    ]);
  const removeItem = (id) =>
  ...
  const handlePrint = () => {
    ...
    invoiceHTML = invoiceHTML
      .replace(
        /<img([^>]*?)alt="Company Logo"([^>]*?)>/g,
        '<img$1alt="Company Logo"$2 class="print-logo">'
      )
      .replace(
        /<img([^>]*?)alt="Signature"([^>]*?)>/g,
        '<img$1alt="Signature"$2 class="print-signature">'
      )
      .replace(
        /<img([^>]*?)alt="Payment QR"([^>]*?)>/g,
        '<img$1alt="Payment QR"$2 class="print-qr">'
      );

    // Replace the header section with our fixed layout
    const billFromSection = `
    <div style="display: flex; justify-content: space-between; margin-bottom: 24px;">
      <div style="flex: 1; max-width: 50%;">
        <h2 style="font-size: 16pt; font-weight: bold; color: #000;">${billFrom.name}</h2>
      ...
      <div style="text-align: right; flex: 1; max-width: 50%;">
        <h3 style="font-size: 18pt; font-weight: bold; color: #000;">INVOICE</h3>
  ...
  `;
    ...
    // Replace the original header with our fixed version
    const headerRegex =
      /<div\s+class="flex\s+justify-between\s+mb-8"[^>]*>[\s\S]*?<\/div>/;
    invoiceHTML = invoiceHTML.replace(headerRegex, billFromSection);
  ...
  const exportToPDF = async () => {
    ...
    const content = `
      <div id="pdf-content" style="font-family: Arial, sans-serif; font-size: 12px; background: white; color: black; padding: 20px; line-height: 1.4;">
        ${
          logoPreview
            ? `<div style="text-align: center; margin-bottom: 20px;"><img src="${logoPreview}" alt="Logo" style="max-height: 30px; height: auto; width: auto; object-fit: contain;" /></div>`
            : ""
        }
        ...
        <div style="margin-bottom: 20px;">
          ...
          ${
            billTo.address
              ? `<p style="color: #555; margin: 2px 0;">${billTo.address}</p>`
              : ""
          }
          ${
            billTo.email
              ? `<p style="color: #555; margin: 2px 0;">${billTo.email}</p>`
              : ""
          }
          ${
            billTo.phone
              ? `<p style="color: #555; margin: 2px 0;">${billTo.phone}</p>`
              : ""
          }
        </div>
          ...
          <tbody>
            ${items
              ...
              <tr style="border-bottom: 1px solid #ddd;">
                ...
                <td style="text-align: right; padding: 8px 0; color: #333;">${
                  item.qty
                }</td>
                <td style="text-align: right; padding: 8px 0; color: #333;">₹${Number(
                  item.rate
                ).toFixed(2)}</td>
                <td style="text-align: right; padding: 8px 0; font-weight: bold; color: #000;">₹${(
                  item.qty * item.rate
                ).toFixed(2)}</td>
              </tr>
            `
        ...
        <div style="text-align: center; margin-top: 24px; padding: 16px; background: #f0f8ff; border: 1px solid #add8e6; border-radius: 6px;">
          ...
          <p style="color: #000; font-weight: bold; margin-top: 10px;">Scan to pay ₹${total.toFixed(
            2
          )}</p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white p-4 md:p-6">
      ...
      <header className="mb-8 text-center">
        <h1 className="text-4xl font-bold text-white mb-2">
          ✨ Navokta Invoice Generator
        </h1>
        <p className="text-slate-300 text-lg">
          Create professional invoices with UPI payment in seconds
        </p>
      </header>
        ...
        <div className="w-full lg:w-1/2 space-y-6">
          <Card title="Bill From" icon={<Building className="text-blue-400" />}>
            ...
            <InputField
              ...
              onChange={(e) =>
                setBillFrom({ ...billFrom, email: e.target.value })
              }
            />
            <InputField
              label="Phone"
            ...
            <InputField
              ...
              onChange={(e) =>
                setBillFrom({ ...billFrom, upiId: e.target.value })
              }
                ...
                {logoPreview && (
                  <div className="relative">
                    <img
                      src={logoPreview}
                      alt="Logo"
                      className="h-12 w-12 object-contain rounded border-2 border-blue-500"
                    />
                    <button
          ...
          <Card
            ...
            icon={<Plus className="text-yellow-400" />}
          >
            {items.map((item) => (
              <div
                key={item.id}
                className="bg-slate-700/50 p-4 rounded-lg mb-4 border border-slate-600"
              >
                <InputField
                  label="Item Name"
                  value={item.name}
                ...
                {items.length > 1 && (
                  <button
                    type="button"
                    className="mt-3 flex items-center gap-1 text-red-400 hover:text-red-300 transition"
            ...
            <button
              type="button"
              className="w-full py-3 flex items-center justify-center gap-2 bg-slate-700 hover:bg-slate-600 rounded-lg border border-slate-600 text-white transition"
              onClick={addItem}
            >
          ...
          >
            ...
            <InputField
              ...
              value={summary.tax}
              onChange={(e) =>
                setSummary({ ...summary, tax: parseFloat(e.target.value) || 0 })
              }
            />
            <InputField
              label="Shipping (₹)"
                ...
                {logoPreview && (
                  <div className="text-center mb-6">
                    <img
                      src={logoPreview}
                      alt="Company Logo"
                      className="h-16 mx-auto object-contain print-logo"
                    />
                  </div>
                )}
                ...
                <table className="w-full mb-6">
                  <thead>
                    <tr className="border-b-2 border-gray-300">
                      <th className="text-left py-3 text-gray-900">
                        Description
                      </th>
                ...
                {signatureImage && (
                  <div className="mb-6">
                    ...
                    <img
                      src={signatureImage}
                      alt="Signature"
                      className="h-12 print-signature"
                    />
                  </div>
                )}
          ...
          <div className="bg-slate-800 rounded-xl w-full max-w-3xl border-2 border-blue-500 shadow-2xl">
            ...
            <div className="p-6">
              ...
              <p className="text-slate-400 text-sm mt-3">
                ✏️ Draw your signature using your mouse or touchscreen
              </p>
